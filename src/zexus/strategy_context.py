...
class ContextStackParser:
    ...
    def _parse_function_statement(self, token, tokens, start_index):
        """Parse statements in function body context"""
        if token.type == LET:
            ...
            return self._parse_try_catch_statement_from_tokens(tokens, start_index)
        else:
            return self._parse_expression_statement(tokens, start_index)

    def _parse_try_catch_statement_from_tokens(self, tokens, start_index):
        """Parse try-catch from token stream"""
